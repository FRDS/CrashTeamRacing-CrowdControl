...
namespace CrowdControl.Games.Packs
{
    ...
    public class CrashTeamRacing : PS1EffectPack
    {
        ...
        private const uint ADDR_CAMFLAG = 0x098028;
        private const uint ADDR_CAMMODE = 0x098053;
        ...
        private uint previousAddress = 0;
        ...
        protected override void StartEffect(EffectRequest request)
        {
            ...
            switch (codeParams[0])
            {
                ...
                case "brake":
                    ...
                    StartTimed(request,
                        () => Connector.Read8(ADDR_TRACK, out byte trackStorage)
                            ...
                            && Connector.Read8(ADDR_PAUSE, out byte p)
                            && p == (byte)PAUSE.Unpaused,
                        () => Connector.Write8(GetSpeedAddress(), 0x00) && Connector.Freeze8(GetSpeedAddress(), 0x00) && SetPreviousAddress(GetSpeedAddress()),
                ...
                case "backcam":
                    ...
                    return;
                case "spectate":
                    Connector.SendMessage(request.DisplayViewer + " wants to spectate");
                    StartTimed(request,
                    () => (Connector.Read32(ADDR_MODE1, out uint gameMode1)
                    && (uint)(gameMode1 & (uint)(MODE1.StartOfRace | MODE1.Loading | MODE1.MainMenu | MODE1.Cutscene | MODE1.AdventureArena)) == 0)
                    && Connector.Read8(ADDR_CAMFLAG, out byte camflag)
                    && camflag != 0x20,
                    () => Connector.Write8(ADDR_CAMFLAG, 0x20),
                    TimeSpan.FromSeconds(15));
                    return;
                case "fps":
                ...
                case "invisible":
                    ...
                    return;
                case "zerograv":
                    Connector.SendMessage(request.DisplayViewer + " took away the gravity");
                    StartTimed(request,
                    () => Connector.Read16(GetGravity(), out ushort gravity)
                    && gravity > 0 && Connector.Write16(GetInstanceBackup(), gravity),
                    () => Connector.Write16(GetGravity(), 0x0000) && Connector.Freeze16(GetInstanceFlag(), 0x0000) && SetPreviousAddress(GetGravity()),
                    TimeSpan.FromSeconds(20));
                    return;
                case "givereward":
        ...
        protected override bool StopEffect(EffectRequest request)
        {
            ...
            switch (request.EffectID)
            {
                ...
                case "invisible":
                    ...
                    return true;
                case "zerograv":
                    Connector.Read16(GetInstanceBackup(), out ushort gravity);
                    Connector.Unfreeze(previousAddress);
                    Connector.Write16(previousAddress, gravity);
                    return true;
                case "minuslap":
        ...
        private uint GetGravity()
        {
            Connector.Read32LE(ADDR_DRIVER1, out uint driverAddress);
            return driverAddress - 0x80000000 + 0x416;
        }
        ...
        private bool SetPreviousAddress(uint address)
        {
